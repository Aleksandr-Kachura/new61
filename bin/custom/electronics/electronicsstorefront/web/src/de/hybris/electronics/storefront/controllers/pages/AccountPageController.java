...
import de.hybris.electronics.storefront.forms.FamilyUpdateProfileForm;
...
public class AccountPageController extends AbstractSearchPageController
{
	...
	private static final String REDIRECT_TO_FAMILY_PAGE = REDIRECT_PREFIX + "/my-account/family";
	private static final String REDIRECT_TO_ORDER_HISTORY_PAGE = REDIRECT_PREFIX + "/my-account/orders";
	private static final String REDIRECT_TO_PDF = REDIRECT_PREFIX + "/my-account/pdf";
	...
	private static final String FAMILY_CMS_PAGE = "family";
	private static final String PDF_CMS_PAGE = "pdf";
    ...
    @Resource(name = "familyManageService")
    private FamilyManageService familyManageService;

    @Resource(name="ElectronicAccountService")
     ...
     * @throws CMSItemNotFoundException
     * this method prepare data for family page
     */
	@RequestMapping(value = "/family", method = RequestMethod.GET)
	@RequireHardLogIn
	public String family(final Model model) throws CMSItemNotFoundException
	{
		model.addAttribute(TITLE_DATA_ATTR, userFacade.getTitles());
		model.addAttribute(new FamilyRegisterForm());
//		model.addAttribute("customers",familyManageService.getAllFamilyMembers(getUser()));
		model.addAttribute("customers",getUser().getFamilyMembers());
		storeCmsPageInModel(model, getContentPageForLabelOrId(FAMILY_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(FAMILY_CMS_PAGE));
		model.addAttribute(BREADCRUMBS_ATTR, accountBreadcrumbBuilder.getBreadcrumbs("text.account.family"));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}
    ...
    @RequireHardLogIn
    public String family(@RequestParam(defaultValue = "uid",required = false) String uid, final Model model) throws CMSItemNotFoundException
    {
        model.addAttribute(TITLE_DATA_ATTR, userFacade.getTitles());
		model.addAttribute("customers",getUser().getFamilyMembers());

		CustomerModel customerModel = familyManageService.getCustomerByUid(uid);

        final FamilyUpdateProfileForm familyUpdateProfileForm = new FamilyUpdateProfileForm();
        familyUpdateProfileForm.setFirstName(customerModel.getName().split(" ")[0]);
        familyUpdateProfileForm.setLastName(customerModel.getName().split(" ")[1]);
        familyUpdateProfileForm.setUid(customerModel.getUid());
        familyUpdateProfileForm.setEmail(customerModel.getUid());
		familyUpdateProfileForm.setTitleCode(customerModel.getTitle().getCode());
        model.addAttribute("familyUpdateProfileForm", familyUpdateProfileForm);
     ...
     * @return
     * @throws CMSItemNotFoundException
     * save family member's changes
     */
    @RequestMapping(value = "/updateFamilyMember", method = RequestMethod.POST)
    @RequireHardLogIn
    public String updateFamilyMember(final FamilyUpdateProfileForm updateForm) throws CMSItemNotFoundException
    {

		FamilyMemberModel familyMemberModel = (FamilyMemberModel) familyManageService.getCustomerByUid(updateForm.getUid());
		TitleModel titleModel = new TitleModel();
		titleModel.setCode(updateForm.getTitleCode());
		familyMemberModel.setTitle(flexibleSearchService.getModelByExample(titleModel));
		familyMemberModel.setName(updateForm.getFirstName() + " " + updateForm.getLastName());
		familyMemberModel.setUid(updateForm.getEmail());
		CustomerModel customerModel =(CustomerModel) userService.getCurrentUser();
		List<CustomerModel> list = new ArrayList<>();
		list.add(customerModel);
		familyMemberModel.setCustomers(list);
		modelService.save(familyMemberModel);

        return REDIRECT_TO_FAMILY_PAGE;
    }

    @RequestMapping(value = "/searchFamilyMember", method = RequestMethod.POST)
    @RequireHardLogIn
    public String searchFamilyMember(@RequestParam(defaultValue = "search",required = true) String search, Model model) throws CMSItemNotFoundException
    {
        model.addAttribute(TITLE_DATA_ATTR, userFacade.getTitles());
        model.addAttribute("customers",familyManageService.getAllFamilyMembers(getUser()));
        List<FamilyElectronicModel> list = familyManageService.searchMembers(search, getUser());
        if (list.size()>0){
            model.addAttribute("nonFamilyMembers",list);
    ...
    @RequestMapping(value = "/createFamilyMember", method = RequestMethod.POST)
    @RequireHardLogIn
    public String createFamilyMember(final FamilyRegisterForm familyRegisterForm) throws CMSItemNotFoundException
    {
		FamilyMemberModel familyMemberModel = new FamilyMemberModel();
		TitleModel titleModel = new TitleModel();
		titleModel.setCode(familyRegisterForm.getTitleCode());
		familyMemberModel.setTitle(flexibleSearchService.getModelByExample(titleModel));
		familyMemberModel.setName(familyRegisterForm.getFirstName() + " " + familyRegisterForm.getLastName());
		familyMemberModel.setUid(familyRegisterForm.getEmail());
		CustomerModel customerModel =(CustomerModel) userService.getCurrentUser();
		List<CustomerModel> list = new ArrayList<>();
		list.add(customerModel);
		familyMemberModel.setCustomers(list);
        modelService.save(familyMemberModel);
		LOG.info(familyRegisterForm.getFirstName());
        return REDIRECT_TO_FAMILY_PAGE;
    }

    /**
     *
     * @param uid unique field
     * @return
     * @throws CMSItemNotFoundException
     * remove family member from familyElectronic
     */
    @RequestMapping(value = "/removeFamilyMember", method = RequestMethod.POST)
    @RequireHardLogIn
    public String removeFamilyMember(@RequestParam final String uid) throws CMSItemNotFoundException
    {
		FamilyMemberModel familyMemberModel = (FamilyMemberModel) familyManageService.getCustomerByUid(uid);
		modelService.remove(familyMemberModel);
        return REDIRECT_TO_FAMILY_PAGE;
    }
     ...
     * @return redirect string
     * @throws CMSItemNotFoundException
     * this method prepare data for pdf page
     */
	@RequestMapping(value = "/pdf")
	@RequireHardLogIn
	public String pdf(final Model model) throws CMSItemNotFoundException
	{
		List<FamilyMemberModel> familyMembers = getUser().getFamilyMembers();
		model.addAttribute("members",familyMembers);
		storeCmsPageInModel(model, getContentPageForLabelOrId(PDF_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(PDF_CMS_PAGE));
		model.addAttribute(BREADCRUMBS_ATTR, accountBreadcrumbBuilder.getBreadcrumbs("text.account.pdf"));
		model.addAttribute(ThirdPartyConstants.SeoRobots.META_ROBOTS, ThirdPartyConstants.SeoRobots.NOINDEX_NOFOLLOW);
		return getViewForPage(model);
	}
	...
	@RequireHardLogIn
	public String editMedia(@RequestParam(value = "mediaCode") final String mediaCode,final Model model) throws CMSItemNotFoundException
	{
		model.addAttribute("file",electronicAccountService.getMedia(mediaCode));
		List<FamilyMemberModel> familyMembers = getUser().getFamilyMembers();
		model.addAttribute("members",familyMembers);
		storeCmsPageInModel(model, getContentPageForLabelOrId(PDF_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(PDF_CMS_PAGE));
     ...
     * @param model
     * @param redirectModel used for FlashMessage
     * @return
     * @throws CMSItemNotFoundException
     * this method used for upload file
     */
	@RequestMapping(value = "/uploadFile", method = RequestMethod.POST)
	public String uploadFileHandler(@RequestParam("file") MultipartFile file,@RequestParam("textMedia") String textMedia,@RequestParam("member") String member,final Model model,RedirectAttributes redirectModel) throws CMSItemNotFoundException{

        CatalogVersionModel catalogVersion = getCmsSiteService().getCurrentCatalogVersion();
		if( !electronicAccountService.uploadPdfFile(file,catalogVersion,textMedia,member))
		{
			GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.ERROR_MESSAGES_HOLDER, "message.valid.error.wrong.type", null);
			return REDIRECT_TO_PDF;
		}
		LOG.info(member);
        storeCmsPageInModel(model, getContentPageForLabelOrId(PDF_CMS_PAGE));
		setUpMetaDataForContentPage(model, getContentPageForLabelOrId(PDF_CMS_PAGE));
		GlobalMessages.addFlashMessage(redirectModel, GlobalMessages.INFO_MESSAGES_HOLDER, "message.file.should.aproved", null);
		model.addAttribute(BREADCRUMBS_ATTR, accountBreadcrumbBuilder.getBreadcrumbs("text.account.family"));
		return REDIRECT_TO_PDF;
	}
     ...
     * this method return family members
     */
	@ResponseBody
	@RequestMapping(value = "/loaddata", method ={ RequestMethod.GET, RequestMethod.POST })
	public HashMap loadData(@RequestParam(value = "mediaCode") final String mediaCode) {
		CustomerData user = getUser();
		HashMap users = electronicAccountService.getFamily(user);
	    return users;
	}

    /**
     *
     * @param mediaCode code selected media
     * @param userCode code attached user
     * @return
     * attach media for selected user
     */
	@RequestMapping(value = "/deleteFile", method ={ RequestMethod.GET, RequestMethod.POST })
	public String AttachFile (@RequestParam(value = "mediaCode") final String mediaCode) {
		electronicAccountService.deleteFile(mediaCode);
		return REDIRECT_TO_PDF;
	}
