package de.hybris.electronics.facades.customer.impl;
...
public class DefaultFamilyCustomerFacade implements CustomerFacade {

    private static final Logger LOG = Logger.getLogger(DefaultFamilyCustomerFacade.class);
    ...
    public void register(final RegisterData registerData) throws DuplicateUidException {
        ...
        FamilyRegisterData familyRegisterData = (FamilyRegisterData) registerData;
        final FamilyElectronicModel newFamilyCustomer = getModelService().create(FamilyElectronicModel.class);
        newFamilyCustomer.setName(getCustomerNameStrategy().getName(familyRegisterData.getFirstName(), familyRegisterData.getLastName()));

        if (StringUtils.isNotBlank(familyRegisterData.getFirstName()) && StringUtils.isNotBlank(familyRegisterData.getLastName())) {
            newFamilyCustomer.setName(getCustomerNameStrategy().getName(familyRegisterData.getFirstName(), familyRegisterData.getLastName()));
            newFamilyCustomer.setFamilyName(familyRegisterData.getFamilyName());
        }
        final TitleModel title = getUserService().getTitleForCode(familyRegisterData.getTitleCode());
        newFamilyCustomer.setTitle(title);
        setUidForRegister(familyRegisterData, newFamilyCustomer);
        newFamilyCustomer.setSessionLanguage(getCommonI18NService().getCurrentLanguage());
        newFamilyCustomer.setSessionCurrency(getCommonI18NService().getCurrentCurrency());
        getCustomerAccountService().register(newFamilyCustomer, familyRegisterData.getPassword());
